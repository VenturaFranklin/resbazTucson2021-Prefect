...
from prefect import Client, Flow, Parameter, apply_map, case, task, unmapped
...
def blurry_map(image, percent_blurry, blur_max):
...
def define_flow():
    with Flow("Image QC Flow") as flow:
        ...
        file_names = Parameter("file_names")
        image = load_image.map(file_names, unmapped(image_work_size))
        result = tissue_segmentation_simple.map(image, unmapped(image_work_size))
        percent_blurry = identifyBlurryRegions.map(
            image,
            unmapped(image_work_size),
            unmapped(blur_radius),
            unmapped(blur_threshold),
            upstream_tasks=[result],
        )
        apply_map(blurry_map, image, percent_blurry, unmapped(blur_max))
    flow.visualize(filename="pipeline_viz_parallel")
    return flow
...
def main(folder, filetype):
    ...
    flow.register(project_name="Image QC", idempotency_key=flow.serialized_hash())
    flow.executor = executor
    client = Client()  # api_server=api_server, api_token=api_token)
    flow_id = client.register(
        flow=flow,
        build=True,
        project_name="Image QC",
    )

...
def getMag(s, params):
    ...
    osh = openslide.OpenSlide(s["fname"])
    mag = osh.properties.get("openslide.objective-power", "NA")
    if (
        mag == "NA"
    ):  # openslide doesn't set objective-power for all SVS files: https://github.com/openslide/openslide/issues/247
...
class BaseImage(dict):
    def __init__(self, fname, fname_outdir=None, params=None):
        dict.__init__(self)

        self["warnings"] = [
            ""
        ]  # this needs to be first key in case anything else wants to add to it
        self["output"] = []
        self["fname"] = fname
        ...
        if fname_outdir is None:
            fname_outdir = os.path.dirname(fname) + "\processed"
            os.makedirs(fname_outdir, exist_ok=True)
        self["outdir"] = fname_outdir
        self["dir"] = os.path.dirname(fname)

        # Can not use this b/c of serialization issues
        # self["os_handle"] = openslide.OpenSlide(fname)
        self["image_base_size"] = openslide.OpenSlide(fname).dimensions
        ...
        self["img_mask_use"] = np.ones(
            self.getImgThumb(self["image_work_size"]).shape[0:2], dtype=bool
        )
        self["img_mask_force"] = []
    ...
    def getImgThumb(self, dim):
        key = "img_" + str(dim)
        if key not in self:
            osh = openslide.OpenSlide(self["fname"])
            if dim.replace(".", "0", 1).isdigit():  # check to see if dim is a number
                dim = float(dim)
                if (
                    dim < 1 and not dim.is_integer()
                ):  # specifying a downscale factor from base
                    new_dim = np.asarray(osh.dimensions) * dim
                    self[key] = np.array(osh.get_thumbnail(new_dim))
                elif (
                    dim < 100
                ):  # assume it is a level in the openslide pyramid instead of a direct request
                    dim = int(dim)
                    ...
                    logging.info(
                        f"{self['filename']} - \t\tcreating image thumb of size {str(dim)}"
                    )
                ...
                if (
                    base_mag != "NA"
                ):  # if base magnification is not known, it is set to NA by basic module
                ...
                output = []
                for x in range(
                    0, dim_base[0], round(win_size * osh.level_downsamples[level])
                ):
                    row_piece = []
                    for y in range(
                        0, dim_base[1], round(win_size * osh.level_downsamples[level])
                    ):
                ...
                output = output[
                    0 : round(dim_base[1] * 1 / down_factor),
                    0 : round(dim_base[0] * 1 / down_factor),
                    :,
                ]
                self[key] = output
            else:
